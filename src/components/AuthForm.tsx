...
import { Heart, Mail, Lock, User, Calendar, Sparkles, ChevronLeft, Loader2, Shield, ChevronDown } from "lucide-react";
...
const COUNTRY_CODES = [
  { code: "+34", flag: "🇪🇸", name: "España", minLen: 9 },
  { code: "+376", flag: "🇦🇩", name: "Andorra", minLen: 6 },
  { code: "+33", flag: "🇫🇷", name: "Francia", minLen: 9 },
  { code: "+44", flag: "🇬🇧", name: "UK", minLen: 10 },
  { code: "+49", flag: "🇩🇪", name: "Alemania", minLen: 10 },
  { code: "+39", flag: "🇮🇹", name: "Italia", minLen: 9 },
  { code: "+351", flag: "🇵🇹", name: "Portugal", minLen: 9 },
  { code: "+52", flag: "🇲🇽", name: "Mexico", minLen: 10 },
  { code: "+54", flag: "🇦🇷", name: "Argentina", minLen: 10 },
  { code: "+57", flag: "🇨🇴", name: "Colombia", minLen: 10 },
  { code: "+56", flag: "🇨🇱", name: "Chile", minLen: 9 },
  { code: "+51", flag: "🇵🇪", name: "Peru", minLen: 9 },
  { code: "+58", flag: "🇻🇪", name: "Venezuela", minLen: 10 },
  { code: "+1", flag: "🇺🇸", name: "EEUU / CA", minLen: 10 },
];
...
function WhatsAppOtpFlow({ onSuccess }: { onSuccess: () => void }) {
  ...
  const formatted = phone.startsWith("+") ? phone : `${countryCode.code}${phone.replace(/\s/g, "")}`;

  const handleSend = async () => {
    const minLen = countryCode.minLen;
    const cleanPhone = phone.replace(/\s/g, "");
    if (!cleanPhone || cleanPhone.length < minLen) return;
    setSending(true);
    setError(null);
    ...
    try {
      ...
      let res: Response;
      try {
        res = await fetch(`${API_URL}/api/auth/otp/send`, {
          method: "POST",
      ...
      if (!res.ok) {
        if (data.code === "BRIDGE_OFFLINE" || data.code === "BRIDGE_UNREACHABLE") {
          setError("WhatsApp Bridge offline. Usa Google o email para entrar.");
          setStep("error");
        } else if (res.status === 429) {
          setError("Demasiados intentos. Espera 10 min e inténtalo de nuevo.");
        } else if (res.status === 400) {
          setError("Numero de telefono invalido. Revisa el formato.");
        } else {
          setError(data.error || "Error enviando codigo. Intenta de nuevo.");
        }
        setSending(false);
        return;
      }
      ...
      if (err?.name === "AbortError") {
        setError("Tiempo de espera agotado. El servidor tarda en responder.");
      } else {
        setError("Error de conexion. Comprueba tu internet e intenta de nuevo.");
      }
    }
    setSending(false);
  };
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-white/40 text-center">Te enviaremos un codigo de verificacion por WhatsApp</p>
      {error && <p className="text-xs text-rose-400 text-center">{error}</p>}
      <div className="flex gap-2">
        ...
        <div className="relative shrink-0">
          <button
            ...
            className="flex items-center gap-1.5 px-3 h-12 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm font-medium text-white/70 hover:bg-white/[0.07] transition min-w-[90px]"
          >
          ...
          {showCountryPicker && (
            <div className="absolute top-14 left-0 z-50 w-56 max-h-64 overflow-y-auto rounded-xl bg-surface-800 border border-white/10 shadow-2xl">
              {COUNTRY_CODES.map((c) => (
                <button
                  key={c.code + c.name}
                  type="button"
                  onClick={() => { setCountryCode(c); setShowCountryPicker(false); }}
                  className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-white/[0.06] transition text-left"
                >
        ...
        <input
          ...
          placeholder={countryCode.code === "+34" ? "612 345 678" : "Numero"}
      ...
      <p className="text-xs text-white/30 text-center">
        {formatted && phone.replace(/\s/g, "").length >= 6 ? `Enviando a: ${formatted}` : "Introduce tu número sin el indicativo de país"}
      </p>
      <button
        onClick={handleSend}
        disabled={sending || phone.replace(/\s/g, "").length < countryCode.minLen}
        className="w-full py-3.5 rounded-xl text-white font-semibold flex items-center justify-center gap-2 disabled:opacity-40 transition-all"
        style={{ background: "#25D366" }}
      >
        {sending ? (
